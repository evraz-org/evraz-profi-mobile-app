...
class ActivityScanResultTransfer : BtsppActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityScanResultTransferBinding.inflate(layoutInflater)
        // 设置自动布局
        ...
        if (_bLockAmount) {
            ...
            //  绑定事件处理
            //  初始化事件
            _tf_amount_watcher = UtilsDigitTextWatcher().set_tf(binding.tfAmountFromScanResultTransfer).set_precision(_asset.getInt("precision"))
        ...
        if (_bLockMemo) {
            ...
            //  备注：用户输入
            binding.layoutMemoInfoAutoInput.visibility = View.GONE
            binding.layoutMemoInfoInput.visibility = View.VISIBLE
        }
