...
class ActivityGatewayWithdraw : BtsppActivity() {
    ...
    private lateinit var _binding: ActivityGatewayWithdrawBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        _binding = ActivityGatewayWithdrawBinding.inflate(layoutInflater)
        ...
        //  back
        _binding.layoutBackFromGatewayWithdraw.setOnClickListener { onBackClicked(null) }
    ...
    private fun onAmountChanged(str_amount: String) {
        ...
        //  无效输入
        if (str_amount == "") {
            _binding.availableOfWithdrawPage.let {
    ...
    private fun initAllUI() {
        ...
        //  可用
        _binding.availableOfWithdrawPage.text = "${_n_available.toPlainString()} ${(_withdrawAssetItem.get("kAppExt") as GatewayAssetItemData).symbol}"
        ...
        //  点击全部
        _binding.totalTextOfWithdrawPage.setOnClickListener {
        ...
        _binding.layoutParentOfWithdrawPage.apply {
            _tf_memo?.let {
    ...
    private fun gotoWithdrawCore() {
        val str_address = _binding.tfWithdrawAddress.text.toString()
        if (str_address == "" || str_address.isEmpty()) {
            showToast(R.string.kVcDWSubmitTipsAddressCannotBeEmpty.xmlstring(this))
            return
        }

        val str_amount = _binding.tfWithdrawAmount.text.toString()
        var str_memo = ""
