...
class ActivityTransferFromBlind : BtsppActivity() {
    ...
    private lateinit var _binding: ActivityTransferFromBlindBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        _binding = ActivityTransferFromBlindBinding.inflate(layoutInflater)
        ...
        //  初始化UI
        _viewBlindInputs = ViewBlindAccountsOrReceipt(this, kBlindItemTypeInput,  _binding.layoutBlindReceiptListFromTransferFromBlind, callback_remove = { _on_remove_input_clicked(it) }, callback_add = { _on_add_one_input_clicked() })
        refreshUI()

        //  选择目标账户箭头颜色
        _binding.imgArrowToAccount.setColorFilter(resources.getColor(R.color.theme01_textColorGray))

        //  选择目标账户事件
        _binding.layoutToAccount.setOnClickListener { onSelectGoalAccount() }

        //  提交事件
        _binding.btnCommit.setOnClickListener { onSubmit() }

        //  返回事件
        _binding.layoutBackFromTransferFromBlind.setOnClickListener { finish() }
    }
    ...
    private fun _draw_ui_to_accounts() {
        if (_to_account != null) {
            _binding.tvToAccountName.text = _to_account!!.getString("name")
            _binding.tvToAccountId.text = _to_account!!.getString("id")
            _binding.tvToAccountName.setTextColor(resources.getColor(R.color.theme01_buyColor))
            _binding.tvToAccountId.setTextColor(resources.getColor(R.color.theme01_textColorMain))
        } else {
            _binding.tvToAccountName.text = resources.getString(R.string.kVcAssetOpCellValueIssueTargetAccountDefault)
            _binding.tvToAccountId.text = ""
            _binding.tvToAccountName.setTextColor(resources.getColor(R.color.theme01_textColorGray))
            _binding.tvToAccountId.setTextColor(resources.getColor(R.color.theme01_textColorGray))
        }
    }
    ...
    private fun _draw_ui_bottom_data() {
        ...
        if (_curr_blind_asset != null) {
            _binding.tvTotalInputValue.text = String.format("%s %s", n_total_input.toPriceAmountString(), _curr_blind_asset!!.getString("symbol"))
        } else {
            _binding.tvTotalInputValue.text = "--"
        }
        ...
        if (n_fee != null) {
            ...
            _binding.tvActualAmount.text = String.format("%s %s", n_final.toPriceAmountString(), _curr_blind_asset!!.getString("symbol"))
        } else {
            _binding.tvActualAmount.text = "--"
        }
    }
