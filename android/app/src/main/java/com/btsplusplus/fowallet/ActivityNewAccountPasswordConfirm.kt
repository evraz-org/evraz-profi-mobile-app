...
class ActivityNewAccountPasswordConfirm : BtsppActivity() {
    ...
    private lateinit var _binding: ActivityNewAccountPasswordConfirmBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 设置自动布局
        _binding = ActivityNewAccountPasswordConfirmBinding.inflate(layoutInflater)
        ...
        when (_scene) {
            kNewPasswordSceneRegAccount -> {
                //  注册账号
                _binding.btnTermsOfService.visibility = View.VISIBLE
                _binding.layoutModifyRange.visibility = View.GONE
                //  UI - 新账号名
                _binding.tvYourAccountNameTitle.text = resources.getString(R.string.kEditPasswordCellTItleYourNewAccountName)
                _binding.tvCurrAccountNameValue.setText(_new_account_name!!)
                _binding.tvCurrAccountNameValue.isEnabled = false
                //  UI - 提交按钮名字
                _binding.btnSubmit.text = resources.getString(R.string.kLoginCellBtnAgreeAndReg)
                //  事件 - 用户协议
                _binding.btnTermsOfService.setOnClickListener { onTermsOfServiceClicked() }
            }
            kNewPasswordSceneChangePassowrd -> {
                //  修改密码
                _binding.btnTermsOfService.visibility = View.INVISIBLE
                _binding.layoutModifyRange.visibility = View.VISIBLE
                //  UI - 账号名
                _binding.tvYourAccountNameTitle.text = resources.getString(R.string.kEditPasswordCellTitleCurrAccountName)
                _binding.tvCurrAccountNameValue.setText(WalletManager.sharedWalletManager().getWalletAccountInfo()!!.getJSONObject("account").getString("name"))
                _binding.tvCurrAccountNameValue.isEnabled = false
                //  UI - 提交按钮名字
                _binding.btnSubmit.text = resources.getString(R.string.kEditPasswordBtnSubmmit)
                //  UI - 修改范围
                _draw_ui_modify_range()
                //  事件 - 选择修改范围
                _binding.layoutModifyRangeCell.setOnClickListener { onModifyRangeClicked() }
                //  UI - 箭头颜色
                _binding.imgArrowModifyRange.setColorFilter(resources.getColor(R.color.theme01_textColorGray))
            }
            kNewPasswordSceneGenBlindAccountBrainKey -> {
                ...
                _binding.layoutModifyRange.visibility = View.GONE
                //  UI - 别名
                _binding.tvYourAccountNameTitle.text = resources.getString(R.string.kVcStCellTitleAliasName)
                _binding.tvCurrAccountNameValue.hint = resources.getString(R.string.kVcStPlaceholderInputAliasName)
                _binding.tvCurrAccountNameValue.isEnabled = true
                //  UI - 提交按钮名字
                _binding.btnSubmit.text = resources.getString(R.string.kEditPasswordBtnCreateBlindAccount)
            }
            else -> assert(false)
        }

        //  事件 - 提交事件
        _binding.btnSubmit.setOnClickListener { onBtnSubmit() }

        //  事件 - 返回
        _binding.layoutBackFromNewAccountPasswordConfirm.setOnClickListener { finish() }
    }
    ...
    private fun _draw_ui_modify_range() {
        when (_curr_modify_range) {
            kModifyAllPermissions -> _binding.tvModifyRangeValue.text = resources.getString(R.string.kEditPasswordCellValueEditRangeOwnerAndActive)
            kModifyOnlyActivePermission -> _binding.tvModifyRangeValue.text = resources.getString(R.string.kEditPasswordCellValueEditRangeOnlyActive)
            kModifyOnlyOwnerPermission -> _binding.tvModifyRangeValue.text = resources.getString(R.string.kEditPasswordCellValueEditRangeOnlyOwner)
            else -> _binding.tvModifyRangeValue.text = ""
        }
    }
    ...
    private fun onBtnSubmit() {
        //  校验参数
        val confirm_password = _binding.tfConfirmPassword.text.toString()
        ...
        when (_scene) {
            ...
            kNewPasswordSceneGenBlindAccountBrainKey -> {
                val str_alias_name = _binding.tvCurrAccountNameValue.text.toString().trim()
