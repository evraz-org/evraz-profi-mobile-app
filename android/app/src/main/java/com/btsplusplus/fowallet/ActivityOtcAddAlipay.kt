...
class ActivityOtcAddAlipay : BtsppActivity() {
    ...
    private lateinit var _binding: ActivityOtcAddAlipayBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        _binding = ActivityOtcAddAlipayBinding.inflate(layoutInflater)
        ...
        // 返回
        _binding.layoutBackFromOtcAddApipay.setOnClickListener { finish() }

        //  提交
        _binding.tvSubmitFromOtcAddAlipay.setOnClickListener { onSubmit(result_promise) }

    }

    private fun onSubmit(result_promise: Promise?) {
        val str_realname = _binding.tfRealname.text.toString()
        val str_account = _binding.tfAccount.text.toString()
