...
class ActivityCreateHtlcContract : BtsppActivity() {
    ...
    private lateinit var _binding: ActivityCreateHtlcContractBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        _binding = ActivityCreateHtlcContractBinding.inflate(layoutInflater)
        ...
        _tv_amount = _binding.etAmountFromCreateHtlcContract
        _tv_preimage_or_hash = if (_mode == EHtlcDeployMode.EDM_PREIMAGE.value) _binding.tvPreimage else _binding.tvHashcode

        //  设置标题栏标题
        if (_lock_field) {
            _binding.titleFromCreateHtlcContract.text = R.string.kVcTitleCreateSubHTLC.xmlstring(this)
        } else {
            _binding.titleFromCreateHtlcContract.text = R.string.kVcTitleCreateHTLC.xmlstring(this)
        }
        ...
        //  返回
        _binding.layoutBackFromCreateHtlcContract.setOnClickListener { finish() }
    ...
    private fun _randomSecurePreimage(): String {
        ...
        return String.format("BTSPP%sPREIMAGE", WalletManager.randomPrivateKeyWIF()).uppercase()
    }
    ...
    private fun initDefaultStatus() {
        ...
        if (_mode == EHtlcDeployMode.EDM_PREIMAGE.value) {
            //  from 原像
            _binding.layoutHashcode.visibility = View.GONE
            _binding.layoutPreimageLength.visibility = View.GONE
            _binding.switchAdvanceSettingOfCreateHtlcContract.setOnCheckedChangeListener { _, isChecked: Boolean ->
                _binding.layoutHashtypeAndExpiryGroup.visibility = if (isChecked) View.VISIBLE else View.GONE
            }
            //  默认原像
            _tv_preimage_or_hash.text = SpannableStringBuilder(_randomSecurePreimage())
        } else {
            //  from 原像哈希
            _binding.layoutPreimage.visibility = View.GONE
            _binding.layoutMoresetting.visibility = View.GONE
            _binding.layoutHashtypeAndExpiryGroup.visibility = View.VISIBLE
        }
        ...
        if (_lock_field) {
            ...
            //  复制粘贴按钮
            _binding.tvCopyPreimage.visibility = View.GONE
            _binding.tvPastePreimageValue.visibility = View.GONE

            //  原像hash ui 默认值 和 不可编辑
            _tv_preimage_or_hash.text = SpannableStringBuilder(preimage_hash.last<String>()!!.uppercase())
            _tv_preimage_or_hash.setTextColor(ContextCompat.getColor(applicationContext,R.color.theme01_textColorGray))
            ...
            _binding.lblHashcode.setTextColor(ContextCompat.getColor(applicationContext,R.color.theme01_textColorGray))
            _binding.tvPreimageLength.setTextColor(ContextCompat.getColor(applicationContext,R.color.theme01_textColorGray))
            _binding.lblPreimageLength.setTextColor(ContextCompat.getColor(applicationContext,R.color.theme01_textColorGray))
            _binding.tvHashtype.setTextColor(ContextCompat.getColor(applicationContext,R.color.theme01_textColorGray))
            _binding.lblHashtype.setTextColor(ContextCompat.getColor(applicationContext,R.color.theme01_textColorGray))
            _binding.tailerArrowHashtype.visibility = View.GONE
            _binding.tailerArrowPreimageLength.visibility = View.GONE

            val tv_to = findViewById<TextView>(R.id.txt_value_to_name)
            tv_to.text = _ref_to!!.getString("name")
            tv_to.setTextColor(ContextCompat.getColor(applicationContext,R.color.theme01_buyColor))
            ...
            //  复制 or 粘贴按钮事件
            if (_mode == EHtlcDeployMode.EDM_PREIMAGE.value) {
                _binding.tvCopyPreimage.setOnClickListener { onCopyButtonClicked() }
            } else {
                _binding.tvPastePreimageValue.setOnClickListener { onPasteButtonClicked() }
            }

            //  原像长度
            _binding.layoutPreimageLength.setOnClickListener {
                var defaultIndex = 0
                val list = JSONArray()
                for (i in 1..256) {
                ...
                ViewDialogNumberPicker(this, R.string.kVcHtlcPlaceholderInputPreimageLength.xmlstring(this), list, "name", defaultIndex) { _index: Int, txt: String ->
                    _binding.tvPreimageLength.text = txt
                    _currPreimageLength = list.getJSONObject(_index).getInt("value")
                }.show()
            }

            //  哈希算法
            _binding.layoutHashtype.setOnClickListener {
                var defaultIndex = 0
        ...
        //  有效期
        _binding.layoutExpiry.setOnClickListener {
            var defaultIndex = 0
            ...
            ViewDialogNumberPicker(this, R.string.kVcHtlcPlaceholderInputExpire.xmlstring(this), _const_expire_list!!, "name", defaultIndex) { _index: Int, txt: String ->
                _binding.tvExpiry.text = txt
                _currExpire = _const_expire_list!!.getJSONObject(_index)
            }.show()
        }

        //  默认值
        _binding.tvHashtype.text = _currHashType!!.getString("name")
        _binding.tvExpiry.text = _currExpire!!.getString("name")
        _binding.tvPreimageLength.text = _currPreimageLength.toString()

        //  创建按钮
        _binding.btnCreateHtlc.setOnClickListener { gotoCreateHTLC() }
    }
    ...
    private fun onAmountChanged(str_amount: String) {
        ...
        if (str_amount == "") {
            ...
            tf.setTextColor(ContextCompat.getColor(applicationContext,R.color.theme01_textColorMain))
            return
        }
        ...
        if (amount > _n_available) {
            ...
            tf.setTextColor(ContextCompat.getColor(applicationContext,R.color.theme01_tintColor))
        } else {
            tf.text = "${_s_available}${symbol}"
            tf.setTextColor(ContextCompat.getColor(applicationContext,R.color.theme01_textColorMain))
        }
    ...
    private fun refreshUI() {
        ...
        if (to != null) {
            ...
            to_txt.setTextColor(ContextCompat.getColor(applicationContext,R.color.theme01_buyColor))
        } else {
            to_txt.text = resources.getString(R.string.kVcTransferTipSelectToAccount)
            to_txt.setTextColor(ContextCompat.getColor(applicationContext,R.color.theme01_textColorGray))
        }
